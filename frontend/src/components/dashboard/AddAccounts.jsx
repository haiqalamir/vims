...
export default function AddAccounts() {
  ...
  // Handle change for form fields; normalize dropdown returns:
  const handleChange = (field, value) => {
    // If value is an object with a value property, extract it.
    const normalizedValue = value && value.value ? value.value : value;
    setAccountDetails((prev) => ({ ...prev, [field]: normalizedValue }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // Log the accountDetails to verify before submission
    console.log("Submitting account details:", accountDetails);

    // Basic front-end validation: all fields are required
    if (
      !accountDetails.name ||
    ...
    try {
      // POST the account details to the accounts API endpoint
      const response = await axios.post("http://127.0.0.1:8000/accounts/", accountDetails);
      ...
      console.error("Error adding account:", error.response ? error.response.data : error);
      toast.error("Error adding account. Please check the console for details.");
    }
  };
        ...
        <div className="content-column">
          <div className="inner-column">
            <div className="list-title">
              ...
              <div className="text">Please fill in the account details below.</div>
            </div>
                          ...
                          <SelectComponent
                            ...
                            value={
                              employmentStatusOptions.find(
                                (opt) => opt.value === accountDetails.employment_status
                              )
                            }
                      ...
                      <div className="form-column col-lg-6">
                        <div className="form_boxes">
                          ...
                          <SelectComponent
                            ...
                            value={
                              roleOptions.find(
                                (opt) => opt.value === accountDetails.role
                              )
                            }
