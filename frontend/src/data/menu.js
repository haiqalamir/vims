export const homeLinks = [
  { href: "/", title: "Home 01" },
  { href: "/home-2", title: "Home 02" },
  { href: "/home-3", title: "Home 03" },
  { href: "/home-4", title: "Home 04" },
  { href: "/home-5", title: "Home 05" },
  { href: "/home-6", title: "Home 06" },
  { href: "/home-7", title: "Home 07" },
  { href: "/home-8", title: "Home 08" },
  { href: "/home-9", title: "Home 09" },
  { href: "/home-10", title: "Home 10" },
];

export const megaMenuData = [
  {
    title: "Inventory List",
    links: [
      { href: "/inventory-list-01", label: "Inventory List v1" },
      { href: "/inventory-list-02", label: "Inventory List v2" },
      { href: "/inventory-map-cards", label: "Map - Cards" },
      { href: "/inventory-map-rows", label: "Map - Rows" },
      { href: "/inventory-sidebar-rows", label: "Sidebar - Rows" },
      { href: "/inventory-sidebar-cards", label: "Sidebar - Cards" },
    ],
  },
  {
    title: "Inventory Single",
    links: [
      { href: "/vehicle-details/1", label: "Inventory Single v1" },
      { href: "/inventory-page-single-v2", label: "Inventory Single v2" },
      { href: "/inventory-page-single-v3", label: "Inventory Single v3" },
      { href: "/inventory-page-single-v4", label: "Inventory Single v4" },
      { href: "/inventory-page-single-v5", label: "Inventory Single v5" },
    ],
  },
  {
    title: "Popular Makes",
    links: [
      { href: "/vehicle-details/1", label: "Audi", inactive: true },
      { href: "/vehicle-details/1", label: "BMW", inactive: true },
      { href: "/vehicle-details/1", label: "Ford", inactive: true },
      { href: "/vehicle-details/1", label: "Honda", inactive: true },
      {
        href: "/vehicle-details/1",
        label: "Land Rover",
        inactive: true,
      },
      {
        href: "/vehicle-details/1",
        label: "Mercedes-Benz",
        inactive: true,
      },
    ],
  },
  {
    title: "Type",
    links: [
      { href: "/vehicle-details/1", label: "Sedan", inactive: true },
      { href: "/vehicle-details/1", label: "SUVs", inactive: true },
      {
        href: "/vehicle-details/1",
        label: "Sport Coupe",
        inactive: true,
      },
      {
        href: "/vehicle-details/1",
        label: "Convertible",
        inactive: true,
      },
      { href: "/vehicle-details/1", label: "Wagon", inactive: true },
    ],
  },
];
export const blogLinks = [
  { href: "/blog-list-01", label: "Blog List 01" },
  { href: "/blog-list-02", label: "Blog List 02" },
  { href: "/blog-list-03", label: "Blog List 03" },
  { href: "/blog-single/1", label: "Blog Single" },
];
export const shopLinks = [
  { href: "/shop-list", label: "Shop List" },
  { href: "/shop-single/1", label: "Shop Single" },
  { href: "/cart", label: "Cart" },
  { href: "/checkout", label: "Checkout" },
];
export const pages = [
  {
    title: "Dashboard",

    iconClass: "fa fa-angle-right",
    links: [
      { title: "Dashboard", href: "/dashboard" },
      { title: "All Vehicle Listing", href: "/my-listings" },
      { title: "Add Listings", href: "/add-listings" },
      { title: "bookeds", href: "/booked" },
      { title: "Saved Search", href: "/saved" },
      { title: "Messages", href: "/messages" },
      { title: "Profile", href: "/profile" },
      { title: "Account List", href: "/account-lists" },
      { title: "Add Account", href: "/add-accounts" },
    ],
  },
  { title: "About", href: "/about" },
  { title: "Services", href: "/contact" },
  { title: "Login", href: "/login" },
  { title: "FAQs", href: "/faq" },
  { title: "Pricing", href: "/pricing" },
  { title: "Terms", href: "/terms" },
  { title: "Team List", href: "/team-list" },
  { title: "Team Single", href: "/team-single/1" },
  { title: "Dealer List", href: "/dealer" },
  { title: "Dealer Single", href: "/dealer-single/1" },
  { title: "Loan Calculator", href: "/loan-calculator" },
  { title: "Compare", href: "/compare" },
  { title: "404", href: "/404" },
  { title: "Invoice", href: "/invoice" },
  { title: "UI Elements", href: "/ui-elements" },
];
